(function(){
  ...
  var html = '<button class="bubble" id="bub" aria-label="Open chat"><svg viewBox="0 0 24 24"><path d="M20 2H4c-1.1 0-2 .9-2 2v18l4-4h14c1.1 0 2-.9 2-2V4c0-1.1-.9-2-2-2z"/></svg></button>'
    ...
    + '<div class="hdr"><div class="logo">W</div><div class="meta"><div class="name">WolvCapital Support</div><div class="sub"><span class="dot"></span>Typically replies instantly</div></div><button class="close" id="cls">\xd7</button></div>'
    + '<div class="msgs" id="msgs"></div>'
    + '<div class="frm"><div class="row"><textarea class="ipt" id="ipt" rows="1" placeholder="Ask about investments, fees, returns..."></textarea><button class="btn" id="snd">Send</button></div>'
    + '<div class="actions"><button class="lnk" id="hum">Talk to a human</button></div></div></div>';
  ...
  var state={visitor:null,conversation:null,opened:false};

  function el(role,content){var d=document.createElement('div');d.className='m '+role;d.textContent=content;return d;}
  function append(m){var d=el(m.role,m.content);if(m.id)d.dataset.id=m.id;msgs.appendChild(d);msgs.scrollTop=msgs.scrollHeight;}
  ...
  async function init(){
    var res=await api({action:'init',session_id:sid,site:SITE,current_page:location.href,referrer:document.referrer,language:navigator.language,timezone:Intl.DateTimeFormat().resolvedOptions().timeZone,screen_width:screen.width,screen_height:screen.height});
    if(res&&res.conversation){
      ...
      (h.messages||[]).forEach(function(m){append(m);});
      if(!h.messages||!h.messages.length){append({role:'ai',content:"Welcome to WolvCapital. I'm your AI assistant — ask me anything about our investment strategies, fees, or how to get started."});}
  ...
  function subscribeRealtime(cfg,convId){
    ...
    var ws=new WebSocket(url);var ref=0;
    ws.onopen=function(){
      ws.send(JSON.stringify({topic:'realtime:public:messages:conversation_id=eq.'+convId,event:'phx_join',payload:{config:{postgres_changes:[{event:'INSERT',schema:'public',table:'messages',filter:'conversation_id=eq.'+convId}]}},ref:String(++ref)}));
      setInterval(function(){try{ws.send(JSON.stringify({topic:'phoenix',event:'heartbeat',payload:{},ref:String(++ref)}));}catch(e){}},25000);
    };
    ws.onmessage=function(ev){
      try{var data=JSON.parse(ev.data);if(data.event==='postgres_changes'&&data.payload&&data.payload.data&&data.payload.data.record){var rec=data.payload.data.record;showTyping(false);append({id:rec.id,role:rec.role,content:rec.content});}}catch(e){}
    };
  }
  ...
  async function send(){
    ...
    append({role:'visitor',content:v});showTyping(true);
    await api({action:'send',conversation_id:state.conversation.id,content:v});
  }

  bub.addEventListener('click',function(){pnl.classList.add('open');if(!state.opened){state.opened=true;init();}setTimeout(function(){ipt.focus();},100);});
  ...
  hum.addEventListener('click',async function(){if(!state.conversation)return;await api({action:'human',conversation_id:state.conversation.id});});
})();
